...
def createDicoRefHyp(dicoCorpus, directTrad) :
    """
        ...
        Arg : dicoBleu = dictionnaire pour score bleu
        key : phrase ref, value : liste phrase hypothèses

        return : Le dictionnaire dicoBleu
    """
...
def dicoSrc(corpusDict) :
    """
        Méthode qui créée un dictionnaire avec 
        avec une liste de phrases source pour chaque
        type de phrase

        Arg : dicoPhrases = key : type phrase
        value : phrase source
...
def dicoDirIndir(corpusDict) :
    """
        Méthode qui créée un dictionnaire avec 
        avec une liste de phrases hypothèse pour chaque
        type de phrase
        
        Arg : dicoPhrases = key : type phrase
        value : phrase hypothèse
...
def dependance(corpusDict) :
    """
        Méthode qui calcule la profondeur des arbres
        syntaxiques de chaque phrase source

        Arg : dicoSources = key : type phrase
        value : phrase source
        listeMoyDepth = liste des profondeurs moyennes
        pour phrase direct / indirect
        listeDepth = Liste des profondeurs de chaque 
        phrase [ [phrases direct] [phrases indirect] ]
...
def treeDepth(root) :
    """
        Méthode récursive qui calcule la profondeur
...
if __name__ == "__main__":
    ...
    w = [0,0]
    w = learn(train, w, 200)
    print("Vecteur de poid ", w, ", taux d'erreur " ,teste(test,w)) """
